...
/**
 * params= { values, files, directory, outputPath }
...
var generateDOCX= function(ind,params,callback){                                                                //log.debug("genDOCX generateDOCX START:",ind,params.files,{});
    if(!params){
        callback({error:{message:"No parameters for generate docx!",userMessage:"Нет данных для создания документа по шаблону!"}}); return;
    }
    if(!params.files||!params.files[ind]){ callback(); return; }
    if(!params.values) {
        callback({error:{message:"No data values for generate docx!",userMessage:"Нет данных для подстановки значений для создания документа по шаблону!"}});
        return;
    }
    ...
    try{//Load the docx file as a binary
        ...
        callback({error:{message:"Failed load template docx file! Reason:"+error.message,
            userMessage:"Не удалось найти (загрузить для обработки) файл шаблона '"+filename+"'!"}});
        return;
    }
    ...
    try{/* render the document (replace all occurences of {<pName>} by values)*/                                //log.debug("genDOCX generateDOCX doc.fileTypeConfig.tagRawXml",doc.fileTypeConfig.tagRawXml,{});
        ...
        callback({error:{message:"Failed replace all occurences in template! Reason:"+error.message,
            userMessage:"Не удалось обработать файл шаблона!"}});
        return;
    ...
    try{// buf is a nodejs buffer, you can either write it to a file or do anything else with it.
        ...
        callback({error:{message:"Failed store result docx file! Reason:"+error.message, userMessage:"Не удалось сохранить файл результата!"}});
        return;
    }
